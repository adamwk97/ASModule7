---
title: "Advanced Statistics Module 7 - Confidence Interval Estimation And introduction to Fundamental of hypothesis testing"
...
We are 99% confident that the population mean falls within `r cineg` and `r cipos`.
...
```
We are 95% confident that the mean value of all greeting cards stored in the store's inventory is between `r cineg` and `r cipos`.
...
Null Hypothesis example: Age has no effect on competency levels with technology.
Alternative Hypothesis Example: Age does have effect on competency levels with technology.
